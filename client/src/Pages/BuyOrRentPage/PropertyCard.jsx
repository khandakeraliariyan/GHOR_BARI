...
const PropertyCard = ({ property }) => {
    ...
    const displayAddress =
        ...
        (property.address && typeof property.address === "object"
            ? [property.address.street, property.address.upazila_id, property.address.district_id, property.address.division_id].filter(Boolean).join(", ")
            : "Unknown location");
        ...
        <div className="max-w-sm rounded-2xl overflow-hidden shadow-lg bg-white group cursor-pointer transition-all duration-300 hover:shadow-2xl border border-gray-100 mx-auto w-full flex flex-col h-[440px]">
            ...
            <div className="relative h-64 overflow-hidden">
                ...
                <div className="absolute top-4 left-4 flex gap-2 items-center">
                    ...
                    >
                        {verifiedFlag ? <><CheckCircle size={12} /> Verified</> : "Unverified"}
                    </span>
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                    <div className="flex justify-between text-white text-sm font-medium">
                        <div className="flex items-center gap-1.5"><Bed size={16} className="opacity-80" /> {beds}</div>
                        <div className="flex items-center gap-1.5"><Bath size={16} className="opacity-80" /> {baths}</div>
            ...
            <div className="p-5 flex-1 flex flex-col justify-between">
                <div>
                    <div className="flex justify-between items-start mb-2">
                        ...
                        <div className="flex items-center gap-1 px-2 py-0.5 rounded ml-2 bg-amber-50">
                            <Star size={14} className="text-amber-500" fill="currentColor" />
                            <span className="text-sm font-bold text-amber-700">
                                {ownerRating ? `${ownerRating}` : "No rating"}
                            </span>
                        </div>
                    </div>
